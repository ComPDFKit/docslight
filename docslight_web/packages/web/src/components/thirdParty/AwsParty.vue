<template>
  <div class="flex <2xl:flex-col">
    ...
    <div class="bg-white rounded-8px p-24px 2xl:(mr-12px w-[calc(50%-6px)]) <2xl:(w-full mb-12px)">
      ...
      <template v-else>
        <div class="text-sm text-[#404653] mb-8px">
          {{ t('third_party_auth.wizards.aws.fields.aws_access_key_id') }}
        </div>
        <el-input v-model="awsAccessKeyId" :placeholder="t('third_party_auth.enter')" class="mb-24px"></el-input>
        <div class="text-sm text-[#404653] mb-8px">
          {{ t('third_party_auth.wizards.aws.fields.aws_secret_access_key') }}
        </div>
        <el-input v-model="awsSecretAccessKey" :placeholder="t('third_party_auth.enter')"></el-input>
        <div class="flex w-full justify-center mt-48px mb-20px">
          <div v-loading="loading" @click="authorizeHandle" :class="awsAccessKeyId && awsSecretAccessKey ? 'hover:bg-[#244FF0] cursor-pointer' : 'bg-[#88A9FC] text-[#FFFFFF66] cursor-not-allowed'"
        ...
        <div class="p-12px">
          <div class="flex">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.aws.setup[0]') }}
              <a target="_blank" class="text-brand-2 underline" href="https://s3.console.aws.amazon.com/s3/">
                {{ t('third_party_auth.wizards.aws.setup[1]') }}
              </a>
          ...
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.aws.setup[2]') }}
            </div>
          </div>
        </div>
      </div>
...
interface AwsFileItem {
  id: string
  name: string
  is_dir: boolean       // true for bucket / folder
  awsType: string       // original type from API: 'bucket' | 'folder' | 'file' | ...
  prefix: string
  bucket_name: string
...
const fileList = ref<AwsFileItem[]>([])
// Dialog list (independent selection state, loaded from API on open)
const dialogFileList = ref<AwsFileItem[]>([])
...
onMounted(async () => {
  const { data: { data } } = await get('/v1/dms/auth/credentials?source=aws_oss')
  ...
  authorize.value = authorizationList.value.aws_oss?.is_active as boolean
  if (authorize.value) {
    await getFileList()
  }
})
...
const unauthorize = async () => {
  ElMessageBox.confirm(t('third_party_auth.unauthorize_tips.aws'), t('third_party_auth.common.unauthorize'), {
    ...
    loading.value = true
    try {
      await _delete('/v1/dms/auth/credentials?source=aws_oss')
...
const authorizeHandle = async () => {
  if (!awsAccessKeyId.value || !awsSecretAccessKey.value) return
  loading.value = true
  try {
...
const mapAwsFiles = (files: any[] = [], level: number, parentId: string | null, bucketName: string): AwsFileItem[] => {
  return files.map((file) => {
    const awsType: string = file.type ?? ''
    return {
      id: file.id || file.key || file.prefix || file.name,
      name: file.name,
      is_dir: ['bucket', 'folder'].includes(awsType),
      awsType,
      prefix: file.prefix || '',
      bucket_name: awsType === 'bucket' ? file.name : bucketName,
...
const fetchAwsFiles = async (
  ...
  const req: {
    source: 'aws_oss'
    args: { mode?: string; bucket?: string; prefix?: string }
  } = { source: 'aws_oss', args: {} }

  if (isRoot) {
    req.args.mode = 'buckets'
    req.args.bucket = 'buckets'
  } else {
    req.args.mode = 'bucket'
    req.args.bucket = bucketName
    req.args.prefix = prefix
  }

  const { data } = await post('/v1/dms/files/list', req)
  return mapAwsFiles(data.data, level, parentId, bucketName)
}

// ─── Main panel: authorized-items → expandable list ──────────────────────────

const mainFolderCache = new Map<string, AwsFileItem[]>()

const buildMainCache = (items: AuthorizedRawItem[], level = 0, parentId: string | null = null, bucketName = ''): AwsFileItem[] => {
  const result: AwsFileItem[] = []
  for (const item of items) {
    const isDir = item.type === 'folder'
    const effectiveBucket = level === 0 ? item.name : bucketName
    const flat: AwsFileItem = {
      id: item.id,
      name: item.name,
      is_dir: isDir,
      awsType: level === 0 ? 'bucket' : isDir ? 'folder' : 'file',
      prefix: '',
    ...
    if (item.children?.length) {
      const children = buildMainCache(item.children, level + 1, item.id, effectiveBucket)
...
const getFileList = async () => {
  try {
    loading.value = true
    const { data: { data } } = await get('/v1/dms/auth/authorized-items?source=aws_oss')
...
const openMainFolder = async (item: AwsFileItem, index: number) => {
  ...
  try {
    ...
    const childList = await fetchAwsFiles(item.level + 1, item.id, item.bucket_name, prefix)
...
const dialogFolderChildrenCache = new Map<string, AwsFileItem[]>()
...
const getAuthorizedIds = (): Set<string> => {
  ...
  const addLeaves = (children: AwsFileItem[]) => {
...
// Load dialog file list (root buckets) from API when dialog opens
watch(dialogVisible, async (val) => {
  if (val) {
    ...
    try {
      ...
      const rootFiles = await fetchAwsFiles(0, null, '', '', true)
...
const getDescendantEndIndex = (list: AwsFileItem[], startIndex: number): number => {
...
const openDialogFolder = async (item: AwsFileItem, index: number) => {
  if (!item.is_dir) return
...
const collectLeafStates = (item: AwsFileItem): { total: number; selected: number } => {
  if (!item.is_dir) {
    return { total: 1, selected: item.selected ? 1 : 0 }
  }
  let children: AwsFileItem[]
...
const getCheckState = (item: AwsFileItem): 'checked' | 'indeterminate' | 'unchecked' => {
  if (!item.is_dir) {
...
const toggleSelect = (item: AwsFileItem) => {
  const idx = dialogFileList.value.indexOf(item)
  if (idx === -1) return
