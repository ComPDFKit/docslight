    ...
    <div class="bg-white rounded-8px p-24px 2xl:(mr-12px w-[calc(50%-6px)]) <2xl:(w-full mb-12px)">
      ...
      <template v-else>
        <div class="text-xs text-[#888C94] mb-10px">
          {{ t('third_party_auth.wizards.notion.fields.integration_token_title') }}
        </div>
        <el-input v-model="notionToken" type="textarea" :rows="6" :placeholder="t('third_party_auth.wizards.notion.fields.integration_token')"></el-input>
        <div class="flex w-full justify-center mt-10px mb-20px">
          <div v-loading="loading" @click="authorizeHandle" :class="notionToken ? 'hover:bg-[#244FF0] cursor-pointer' : 'bg-[#88A9FC] text-[#FFFFFF66] cursor-not-allowed'"
        ...
        <div class="p-12px">
          <div class="flex">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.notion.setup[0]') }}
              <a target="_blank" class="text-brand-2 underline" href="https://www.notion.so/my-integrations">
                {{ t('third_party_auth.wizards.notion.setup[1]') }}
              </a>
          ...
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.notion.setup[2]') }}
            </div>
          </div>
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.notion.setup[3]') }}
            </div>
          </div>
        </div>
      </div>
...
type NotionRequestMode = 'workspace' | 'database' | 'page_blocks' | 'property'
type NotionNodeType = 'database' | 'page_root' | 'page' | 'file'
...
const fileList = ref<NotionFileItem[]>([])
const dialogFileList = ref<NotionFileItem[]>([])
...
onMounted(async () => {
  const { data: { data } } = await get('/v1/dms/auth/credentials?source=notion')
  ...
  authorize.value = authorizationList.value.notion?.is_active as boolean
  if (authorize.value) {
    await getFileList()
  }
})
...
const unauthorize = async () => {
  ElMessageBox.confirm(t('third_party_auth.unauthorize_tips.notion'), t('third_party_auth.common.unauthorize'), {
    ...
    loading.value = true
    try {
      await _delete('/v1/dms/auth/credentials?source=notion')
...
const authorizeHandle = async () => {
  if (!notionToken.value) return
  loading.value = true
  try {
    const req = JSON.stringify({
      source: 'notion',
      credentials: { token: notionToken.value }
    })
...
const getFilePropertyName = (properties?: Record<string, NotionProperty>): string | null => {
  if (!properties || typeof properties !== 'object') return null
  for (const property of Object.values(properties)) {
    if (!property) continue
    if (property.type === 'file' || property.type === 'files') {
...
const mapNotionFiles = (
  files: NotionRawItem[] = [],
  mode: NotionRequestMode,
  level: number,
  ...
  if (mode === 'workspace') {
    return files.map((file) => {
      ...
      const nodeType: NotionNodeType = isDatabase ? 'database' : 'file'
      const dynamicPropertyName = isDatabase ? getFilePropertyName(file?.properties) : null
      return {
        id: String(file?.id || file?.page_id || file?.database_id || file?.name),
        name: String(file?.title || file?.name || file?.id || 'Untitled'),
        kind,
        node_type: nodeType,
        level,
        parent_id: parentId,
        ...
        expanded: false,
        selected: false,
        file_property_name: dynamicPropertyName || undefined,
      }
    })
  }
  ...
  if (mode === 'database') {
    return files.map((file) => ({
      id: String(file?.id || file?.page_id || file?.name),
      name: String(file?.title || file?.name || file?.id || 'Untitled'),
      kind: String(file?.kind || 'page'),
      node_type: 'page' as NotionNodeType,
      level,
      parent_id: parentId,
      is_expandable: true,
      expanded: false,
      selected: false,
      file_property_name: filePropertyName || undefined,
    }))
  }
  ...
  if (mode === 'page_blocks') {
    return files.map((file) => {
      ...
      return {
        ...
        node_type: (isDir ? 'page_root' : 'file') as NotionNodeType,
        level,
        parent_id: parentId,
        is_expandable: isDir,
        expanded: false,
  ...
  return files.map((file) => ({
    id: String(file?.id || file?.name || file?.url),
    name: String(file?.name || (file as any)?.file_name || file?.title || file?.id || 'Untitled'),
    kind: String(file?.kind || 'file'),
    node_type: 'file' as NotionNodeType,
    level,
    parent_id: parentId,
    is_expandable: false,
    expanded: false,
...
const fetchNotionFiles = async (
  ...
  id?: string,
  filePropertyName?: string | null
): Promise<NotionFileItem[]> => {
  const req: {
    source: 'notion'
    args: { mode: NotionRequestMode; database_id?: string; page_id?: string; property_name?: string }
  } = { source: 'notion', args: { mode } }

  if (mode === 'database' && id) req.args.database_id = id
  if ((mode === 'page_blocks' || mode === 'property') && id) req.args.page_id = id
  if (mode === 'property') {
    if (!filePropertyName) return []
    req.args.property_name = filePropertyName
  }

  const { data } = await post('/v1/dms/files/list', req)
  return mapNotionFiles(data.data, mode, level, parentId, filePropertyName)
}

// ─── Main panel: authorized-items → expandable list ──────────────────────────

const mainFolderCache = new Map<string, NotionFileItem[]>()

const buildMainCache = (items: AuthorizedRawItem[], level = 0, parentId: string | null = null): NotionFileItem[] => {
  const result: NotionFileItem[] = []
  for (const item of items) {
    const isExpandable = item.type === 'folder'
    const flat: NotionFileItem = {
      id: item.id,
      name: item.name,
      kind: isExpandable ? 'database' : 'file',
      node_type: isExpandable ? 'database' : 'file',
      level,
      parent_id: parentId,
...
const getFileList = async () => {
  try {
    loading.value = true
    const { data: { data } } = await get('/v1/dms/auth/authorized-items?source=notion')
...
const openMainFolder = async (item: NotionFileItem, index: number) => {
  ...
  let mode: NotionRequestMode | null = null
  if (item.node_type === 'database') mode = 'database'
  else if (item.node_type === 'page_root') mode = 'page_blocks'
  else if (item.node_type === 'page') mode = 'property'
  if (!mode) return
  try {
    loading.value = true
    const childList = await fetchNotionFiles(mode, item.level + 1, item.id, item.id, item.file_property_name)
...
const dialogFolderChildrenCache = new Map<string, NotionFileItem[]>()
...
const getAuthorizedIds = (): Set<string> => {
  ...
  const addLeaves = (children: NotionFileItem[]) => {
...
watch(dialogVisible, async (val) => {
  if (val) {
    ...
    try {
      ...
      const rootFiles = await fetchNotionFiles('workspace', 0, null)
...
const getDescendantEndIndex = (list: NotionFileItem[], startIndex: number): number => {
  ...
  if (endIndex > index) {
    ...
    for (let i = removed.length - 1; i >= 0; i--) {
      if (removed[i].is_expandable && removed[i].expanded) {
        const subChildren: NotionFileItem[] = []
...
const openDialogFolder = async (item: NotionFileItem, index: number) => {
  if (!item.is_expandable) return
...
const collectLeafStates = (item: NotionFileItem): { total: number; selected: number } => {
  if (!item.is_expandable) {
    return { total: 1, selected: item.selected ? 1 : 0 }
  }
  let children: NotionFileItem[]
...
const getCheckState = (item: NotionFileItem): 'checked' | 'indeterminate' | 'unchecked' => {
  if (!item.is_expandable) {
...
const toggleSelect = (item: NotionFileItem) => {
  const idx = dialogFileList.value.indexOf(item)
  if (idx === -1) return
