    ...
    <div class="bg-white rounded-8px p-24px 2xl:(mr-12px w-[calc(50%-6px)]) <2xl:(w-full mb-12px)">
      ...
      <template v-else>
        <div class="text-sm text-[#404653] mb-10px">
          {{ t('third_party_auth.wizards.gmail.fields.gmail_account_email') }}
        </div>
        <el-input v-model="gmailUsername" :placeholder="t('third_party_auth.enter')"></el-input>
        <div class="text-sm text-[#404653] mb-10px mt-24px">
          {{ t('third_party_auth.wizards.gmail.fields.app_specific_password') }}
        </div>
        <el-input v-model="gmailPassword" show-password type="password" :placeholder="t('third_party_auth.enter')"></el-input>
        <div class="flex w-full justify-center mt-48px mb-20px">
          <div v-loading="loading" @click="authorizeHandle" :class="gmailUsername && gmailPassword ? 'hover:bg-[#244FF0] cursor-pointer' : 'bg-[#88A9FC] text-[#FFFFFF66] cursor-not-allowed'"
        ...
        <div class="p-12px">
          <div class="flex">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.gmail.setup[0]') }}
              <a target="_blank" class="text-brand-2 underline" href="https://myaccount.google.com/security">
                {{ t('third_party_auth.wizards.gmail.setup[1]') }}
              </a>
          ...
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.gmail.setup[2]') }}
            </div>
          </div>
        </div>
      </div>
...
const { t } = useI18n()
const gmailUsername = ref('')
const gmailPassword = ref('')
...
type GmailRequestMode = 'home' | 'mailbox' | 'email'
type GmailNodeType = 'mailbox' | 'email' | 'attachment'

interface GmailFileItem {
  id: string
  name: string
  node_type: GmailNodeType
  level: number
...
const fileList = ref<GmailFileItem[]>([])
const dialogFileList = ref<GmailFileItem[]>([])
...
onMounted(async () => {
  const { data: { data } } = await get('/v1/dms/auth/credentials?source=gmail')
  ...
  authorize.value = authorizationList.value.gmail?.is_active as boolean
  if (authorize.value) {
    await getFileList()
  }
})
...
const unauthorize = async () => {
  ElMessageBox.confirm(t('third_party_auth.unauthorize_tips.gmail'), t('third_party_auth.common.unauthorize'), {
    ...
    loading.value = true
    try {
      await _delete('/v1/dms/auth/credentials?source=gmail')
...
const authorizeHandle = async () => {
  if (!gmailUsername.value || !gmailPassword.value) return
  loading.value = true
  try {
    const req = JSON.stringify({
      source: 'gmail',
      credentials: {
        username: gmailUsername.value,
...
const getNodeTypeByMode = (mode: GmailRequestMode): GmailNodeType => {
  if (mode === 'home') return 'mailbox'
  if (mode === 'mailbox') return 'email'
  return 'attachment'
}

const getRequestModeByNodeType = (nodeType: GmailNodeType): GmailRequestMode | null => {
  if (nodeType === 'mailbox') return 'mailbox'
...
const mapGmailFiles = (
  files: any[] = [],
  mode: GmailRequestMode,
  parentMailbox = '',
  level: number,
  parentId: string | null
): GmailFileItem[] => {
  const nodeType = getNodeTypeByMode(mode)
  const isExpandable = nodeType !== 'attachment'

  return files.map((file) => {
    const mailbox = nodeType === 'mailbox'
      ? String(file?.id ?? file?.mailbox ?? file?.name ?? file)
      : parentMailbox
    const uid = String(file?.uid ?? file?.id ?? '')
    const name = String(file?.name ?? file?.subject ?? file?.fileName ?? file?.filename ?? file?.mailbox ?? file?.id ?? file)

    return {
      id: String(file?.id ?? file?.uid ?? file?.attachment_id ?? file?.name ?? file),
      name,
      node_type: nodeType,
      level,
      parent_id: parentId,
...
const fetchGmailFiles = async (
  mode: GmailRequestMode,
  level: number,
  parentId: string | null,
  mailbox = '',
  uid = ''
): Promise<GmailFileItem[]> => {
  const req: {
    source: 'gmail'
    args: { mode: GmailRequestMode; mailbox?: string; start?: string; end?: string; uid?: string }
  } = { source: 'gmail', args: { mode } }

  if (mode === 'mailbox') {
    const range = getDefaultDateRange()
    req.args.mailbox = mailbox
    req.args.start = range.start
    req.args.end = range.end
  }
  if (mode === 'email') {
    req.args.mailbox = mailbox
    req.args.uid = uid
  ...
  return mapGmailFiles(data.data, mode, mailbox, level, parentId)
}

// ─── Main panel: authorized-items → expandable list ──────────────────────────

const mainFolderCache = new Map<string, GmailFileItem[]>()

const buildMainCache = (items: AuthorizedRawItem[], level = 0, parentId: string | null = null): GmailFileItem[] => {
  const result: GmailFileItem[] = []
  for (const item of items) {
    const isExpandable = item.type === 'folder'
    const flat: GmailFileItem = {
      id: item.id,
      name: item.name,
      node_type: isExpandable ? 'mailbox' : 'attachment',
      level,
      parent_id: parentId,
      is_expandable: isExpandable,
      mailbox: isExpandable ? item.id : '',
      uid: '',
      expanded: false,
      selected: false
    }
...
const getFileList = async () => {
  try {
    loading.value = true
    const { data: { data } } = await get('/v1/dms/auth/authorized-items?source=gmail')
...
const openMainFolder = async (item: GmailFileItem, index: number) => {
  ...
  const mode = getRequestModeByNodeType(item.node_type)
  if (!mode) return
  try {
    loading.value = true
    const childList = await fetchGmailFiles(mode, item.level + 1, item.id, item.mailbox, item.uid)
...
const dialogFolderChildrenCache = new Map<string, GmailFileItem[]>()
...
const getAuthorizedIds = (): Set<string> => {
  ...
  const addLeaves = (children: GmailFileItem[]) => {
...
watch(dialogVisible, async (val) => {
  if (val) {
    ...
    try {
      ...
      const rootFiles = await fetchGmailFiles('home', 0, null)
...
const getDescendantEndIndex = (list: GmailFileItem[], startIndex: number): number => {
...
const collapseDialogFolder = (index: number) => {
  ...
  if (endIndex > index) {
    ...
    // For expanded subfolders, split their children into separate cache entries
    for (let i = removed.length - 1; i >= 0; i--) {
      if (removed[i].is_expandable && removed[i].expanded) {
        const subChildren: GmailFileItem[] = []
...
const openDialogFolder = async (item: GmailFileItem, index: number) => {
  if (!item.is_expandable) return
...
const collectLeafStates = (item: GmailFileItem): { total: number; selected: number } => {
  if (!item.is_expandable) {
    return { total: 1, selected: item.selected ? 1 : 0 }
  }
  let children: GmailFileItem[]
  const directLevel = item.level + 1
  if (item.expanded) {
    const idx = dialogFileList.value.indexOf(item)
    if (idx === -1) {
      // Item has stale expanded flag (in cache, not visible); fall back to cache
...
const getCheckState = (item: GmailFileItem): 'checked' | 'indeterminate' | 'unchecked' => {
  if (!item.is_expandable) {
...
const toggleSelect = (item: GmailFileItem) => {
  const idx = dialogFileList.value.indexOf(item)
  if (idx === -1) return
