    ...
    <div class="bg-white rounded-8px p-24px 2xl:(mr-12px w-[calc(50%-6px)]) <2xl:(w-full mb-12px)">
      ...
      <template v-else>
        <div class="text-sm text-[#404653] mb-10px">
          {{ t('third_party_auth.wizards.gsc.fields.hmac_access_key') }}
        </div>
        <el-input v-model="gcsUsername" :placeholder="t('third_party_auth.enter')"></el-input>
        <div class="text-sm text-[#404653] mb-10px mt-24px">
          {{ t('third_party_auth.wizards.gsc.fields.hmac_secret_key') }}
        </div>
        <el-input v-model="gcsPassword" show-password type="password" :placeholder="t('third_party_auth.enter')"></el-input>
        <div class="flex w-full justify-center mt-48px mb-20px">
          <div v-loading="loading" @click="authorizeHandle" :class="gcsUsername && gcsPassword ? 'hover:bg-[#244FF0] cursor-pointer' : 'bg-[#88A9FC] text-[#FFFFFF66] cursor-not-allowed'"
        ...
        <div class="p-12px">
          <div class="flex">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.gsc.setup[0]') }}
              <a target="_blank" class="text-brand-2 underline" href="https://console.cloud.google.com/">
                {{ t('third_party_auth.wizards.gsc.setup[1]') }}
              </a>
          ...
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.gsc.setup[2]') }}
            </div>
          </div>
        </div>
      </div>
...
const { t } = useI18n()
const gcsUsername = ref('')
const gcsPassword = ref('')
...
const fileList = ref<GcsFileItem[]>([])
const dialogFileList = ref<GcsFileItem[]>([])
...
onMounted(async () => {
  const { data: { data } } = await get('/v1/dms/auth/credentials?source=gcs')
  ...
  authorize.value = authorizationList.value.gcs?.is_active as boolean
  if (authorize.value) {
    await getFileList()
  }
})
...
const unauthorize = async () => {
  ElMessageBox.confirm(t('third_party_auth.unauthorize_tips.gcs'), t('third_party_auth.common.unauthorize'), {
    ...
    loading.value = true
    try {
      await _delete('/v1/dms/auth/credentials?source=gcs')
...
const authorizeHandle = async () => {
  if (!gcsUsername.value || !gcsPassword.value) return
  loading.value = true
  try {
    const req = JSON.stringify({
      source: 'gcs',
      credentials: {
        access_key: gcsUsername.value,
        secret_key: gcsPassword.value
      }
    })
...
const mapGcsFiles = (files: any[] = [], level: number, parentId: string | null, bucketName: string): GcsFileItem[] => {
  return files.map((file) => {
    const fileType: string = file.type ?? ''
    return {
      id: file.id || file.key || file.prefix || file.name,
      name: file.name,
      is_dir: ['bucket', 'folder'].includes(fileType),
      prefix: file.prefix || '',
      bucket_name: fileType === 'bucket' ? file.name : bucketName,
...
const fetchGcsFiles = async (
  ...
  return mapGcsFiles(data.data, level, parentId, bucketName)
}

// ─── Main panel: authorized-items → expandable list ──────────────────────────

const mainFolderCache = new Map<string, GcsFileItem[]>()

const buildMainCache = (items: AuthorizedRawItem[], level = 0, parentId: string | null = null, bucketName = ''): GcsFileItem[] => {
  const result: GcsFileItem[] = []
  for (const item of items) {
    const isDir = item.type === 'folder'
    const effectiveBucket = level === 0 ? item.name : bucketName
    const flat: GcsFileItem = {
      id: item.id,
      name: item.name,
      is_dir: isDir,
      prefix: '',
...
const getFileList = async () => {
  try {
    loading.value = true
    const { data: { data } } = await get('/v1/dms/auth/authorized-items?source=gcs')
...
const openMainFolder = async (item: GcsFileItem, index: number) => {
  ...
  try {
    loading.value = true
    const prefix = item.bucket_name === item.name && item.level === 0 ? '' : item.prefix
    const childList = await fetchGcsFiles(item.level + 1, item.id, item.bucket_name, prefix)
...
const dialogFolderChildrenCache = new Map<string, GcsFileItem[]>()
...
const getAuthorizedIds = (): Set<string> => {
  ...
  const addLeaves = (children: GcsFileItem[]) => {
...
watch(dialogVisible, async (val) => {
  if (val) {
    ...
    try {
      ...
      const rootFiles = await fetchGcsFiles(0, null, '', '', true)
...
const getDescendantEndIndex = (list: GcsFileItem[], startIndex: number): number => {
...
const openDialogFolder = async (item: GcsFileItem, index: number) => {
  if (!item.is_dir) return
  ...
  try {
    loading.value = true
    // bucket level: prefix='', folder level: use item.prefix
    const prefix = item.bucket_name === item.name && item.level === 0 ? '' : item.prefix
    const childList = await fetchGcsFiles(item.level + 1, item.id, item.bucket_name, prefix)
    if (childList.length === 0) {
...
const collectLeafStates = (item: GcsFileItem): { total: number; selected: number } => {
  if (!item.is_dir) {
    return { total: 1, selected: item.selected ? 1 : 0 }
  }
  let children: GcsFileItem[]
...
const getCheckState = (item: GcsFileItem): 'checked' | 'indeterminate' | 'unchecked' => {
  if (!item.is_dir) {
...
const toggleSelect = (item: GcsFileItem) => {
  const idx = dialogFileList.value.indexOf(item)
  if (idx === -1) return
