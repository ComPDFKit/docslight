<template>
  <div class="flex <2xl:flex-col">
    ...
    <div class="bg-white rounded-8px p-24px 2xl:(mr-12px w-[calc(50%-6px)]) <2xl:(w-full mb-12px)">
      ...
      <template v-else>
        <div class="text-xs text-[#888C94] mb-10px">
          {{ t('third_party_auth.wizards.google_drive.fields.service_account_json') }}
        </div>
        <el-input v-model="google_drive" type="textarea" placeholder='{"type": "service_account", ...}' :rows="6"></el-input>
        <div class="flex w-full justify-center mt-10px mb-20px">
          <div v-loading="loading" @click="authorizeHandle" :class="google_drive ? 'hover:bg-[#244FF0] cursor-pointer' : 'bg-[#88A9FC] text-[#FFFFFF66] cursor-not-allowed'"
        ...
        <div class="p-12px">
          <div class="flex">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.google_drive.setup[0]') }}
              <a target="_blank" class="text-brand-2 underline" href="https://console.cloud.google.com/apis/credentials">
                {{ t('third_party_auth.wizards.google_drive.setup[1]') }}
              </a>
          ...
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.google_drive.setup[2]') }}
            </div>
          </div>
          <div class="flex mt-8px">
            ...
            <div class="text-xs text-[#404653]">
              {{ t('third_party_auth.wizards.google_drive.setup[3]') }}
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="w-full flex flex-col gap-8px">
          ...
          >
            ...
            <div class="flex items-center shrink-0 ml-8px cursor-pointer" @click.stop="toggleSelect(file)">
              <Checked v-if="getCheckState(file) === 'checked'" class="1 w-16px h-16px" />
              <Indeterminate v-else-if="getCheckState(file) === 'indeterminate'" class="2 w-16px h-16px" />
              <Check v-else class="3 w-16px h-16px" />
            </div>
          </div>
        </div>
      </div>
...
const { t } = useI18n()
const google_drive = ref('')
const loading = ref(false)
const authorize = ref(false)
const dialogVisible = ref(false)

// Flat list for main panel (read-only, from authorized-items)
const fileList = ref<DriveFileItem[]>([])
// Separate list for the dialog with independent selection state
const dialogFileList = ref<DriveFileItem[]>([])
 ...
 *
 * Strategy: use parent_id to reconstruct the hierarchy.
 * A folder appears as a container even if it is not itself selected,
 * so that its selected children are not orphaned.
 * Only nodes that are selected (or have selected descendants) end up in
 * the final output.
 */
const buildAuthorizedTree = (): AuthorizedItem[] => {
  const list = dialogFileList.value

  // Helper: recursively build children for a given parentId
...
onMounted(async () => {
  const { data: { data  } } = await get('/v1/dms/auth/credentials?source=google_drive')
  ...
  authorize.value = authorizationList.value.google_drive?.is_active as boolean
  if (authorize.value) {
    await getFileList()
  }
})
...
const unauthorize = async () => {
  ElMessageBox.confirm(t('third_party_auth.unauthorize_tips.google_drive'), t('third_party_auth.common.unauthorize'), {
    confirmButtonText: t('third_party_auth.common.ok'),
    cancelButtonText: t('third_party_auth.common.cancel'),
    type: 'warning',
    customClass: 'delete-file'
    ...
    loading.value = true
    try {
      await _delete('/v1/dms/auth/credentials?source=google_drive')
...
const authorizeHandle = async () => {
  if (!google_drive.value) return
  loading.value = true
  try {
    let parsedJson
    try {
      parsedJson = JSON.parse(google_drive.value)
    } catch {
      ElMessage.error(t('third_party_auth.invalid_json'))
      loading.value = false
      return
    }
...
interface DriveFileItem {
  ...
  expanded?: boolean
  selected?: boolean  // true = checked, false = unchecked, undefined = default unchecked
}
...
/**
 * Build the flat root list and populate mainFolderCache with children.
 * Only root-level items go into fileList initially; children are cached for expand.
 */
const mainFolderCache = new Map<string, DriveFileItem[]>()

const buildMainCache = (items: AuthorizedRawItem[], level = 0, parentId: string | null = null): DriveFileItem[] => {
  const result: DriveFileItem[] = []
  for (const item of items) {
    const flat: DriveFileItem = {
      id: item.id,
      name: item.name,
...
const getFileList = async () => {
  try {
    loading.value = true
    const { data: { data } } = await get('/v1/dms/auth/authorized-items?source=google_drive')
...
const openMainFolder = async (item: DriveFileItem, index: number) => {
  if (!item.is_dir) return
  if (item.expanded) {
    // Collapse: remove visible descendants
  ...
  try {
    loading.value = true
    const childList = await fetchFolderFiles(item.id, item.level + 1, item.id)
...
// Separate cache for dialog so selection state is independent
const dialogFolderChildrenCache = new Map<string, DriveFileItem[]>()
// Leaf IDs from the last saved authorized-items response (folders that are containers are excluded)
const dialogSavedIds = new Set<string>()

// Collect only leaf-node IDs from the saved authorized-items tree.
// Folders that appear as containers (i.e. have children in mainFolderCache) are NOT included —
// their checked/indeterminate state is derived by collectLeafStates at render time.
const getAuthorizedIds = (): Set<string> => {
  const ids = new Set<string>()
  for (const item of fileList.value) {
    // Root item is a leaf if it's not a folder-container in the cache
    if (!mainFolderCache.has(item.id)) ids.add(item.id)
  }
  const addLeaves = (children: DriveFileItem[]) => {
    for (const child of children) {
      if (mainFolderCache.has(child.id)) {
        // This child is itself a folder-container; recurse into its children
...
const mapDriveFiles = (files: any[] = [], level: number, parentId: string | null): DriveFileItem[] => {
...
const fetchFolderFiles = async (
  ...
  const args: { folder_id: string; shared_with_me?: boolean } = { folder_id: folderId }
  if (isRoot) args.shared_with_me = true
  const req = { source: 'google_drive' as const, args }
  const { data } = await post('/v1/dms/files/list', req)
  return mapDriveFiles(data.data, level, parentId)
}

// Load dialog file list from API when dialog opens
watch(dialogVisible, async (val) => {
  if (val) {
    ...
    try {
      ...
      const rootFiles = await fetchFolderFiles(rootFolderId, 0, null, true)
...
const getDescendantEndIndex = (list: DriveFileItem[], startIndex: number): number => {
  ...
  if (endIndex > index) {
    ...
    for (let i = removed.length - 1; i >= 0; i--) {
      if (removed[i].is_dir && removed[i].expanded) {
        const subChildren: DriveFileItem[] = []
...
const openDialogFolder = async (item: DriveFileItem, index: number) => {
  if (!item.is_dir) return
  ...
  try {
    ...
    // Store in dialog cache; each child's selected is looked up from the saved leaf IDs
...
/**
 * Returns the visible descendants' indices of the item at startIndex in dialogFileList.
 */
...
// Recursively collect { total, selected } leaf counts for an item,
// consulting dialogFolderChildrenCache for collapsed folders.
const collectLeafStates = (item: DriveFileItem): { total: number; selected: number } => {
  if (!item.is_dir) {
    return { total: 1, selected: item.selected ? 1 : 0 }
  }
  // Get children: visible list if expanded, cache if collapsed
  let children: DriveFileItem[]
  if (item.expanded) {
    ...
    children = dialogFolderChildrenCache.get(item.id)?.filter(c => c.level === item.level + 1) ?? []
  }
...
const getCheckState = (item: DriveFileItem): 'checked' | 'indeterminate' | 'unchecked' => {
  if (!item.is_dir) {
    return item.selected ? 'checked' : 'unchecked'
  }
  // If folder has never been expanded and has no cache, fall back to its own flag
...
/**
 * Toggle the selection of a node and propagate to all visible descendants.
 */
...
const toggleSelect = (item: DriveFileItem) => {
  const idx = dialogFileList.value.indexOf(item)
  if (idx === -1) return

  const currentState = getCheckState(item)
  // If checked or indeterminate → uncheck; if unchecked → check
...
/**
 * Deselect all items in the dialog list.
 */
const deSelectAll = () => {
...
/**
 * Whether any item in the dialog is selected.
 */
